...
# All 14 pages with webflow_item_id
pages = [
...
print(f"🔄 Regenerating {len(pages)} pages with new subheadline field...\n")

for i, (node_id, keyword) in enumerate(pages, 1):
    print(f"\n[{i}/{len(pages)}] 📝 {keyword}")
    print(f"    Generating content...")
    subprocess.run(["python", "sei_unified.py", "generate", node_id])

    print(f"    Publishing to Webflow...")
    subprocess.run(["python", "sei_unified.py", "publish", node_id])

    print(f"    ✅ Done!")
    time.sleep(1)  # Small delay between pages

print(f"\n🎉 All {len(pages)} pages regenerated with subheadline!")